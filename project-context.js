(() => {
  ...
  if(isContentEditor){
    ...
    const placePreview=()=>{
      ...
      if(preview.parentElement!==right){preview.classList.remove('btn','ghost');preview.textContent='Preview ↗';const site=right.querySelector('#publishedLink,[data-published-link],a[href*="/p/"]');if(site)right.insertBefore(preview,site);else right.prepend(preview)}
