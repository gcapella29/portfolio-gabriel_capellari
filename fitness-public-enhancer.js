(() => {
  ...
  function enhance(result){
    ...
    if(method){
      const methodData=content('wsop'), hasMethod=Boolean(cleanText(local(methodData,'description',locale))) || (Array.isArray(methodData.points)&&methodData.points.some(x=>cleanText(typeof x==='string'?x:x?.value)));
      if(!hasMethod){method.hidden=true;} else {
        setHeading(method, locale==='pt'?'Do primeiro contato à evolução':'From first contact to progress', locale==='pt'?'Meu método':'My method');
    ...
    if(results&&items.length){
      ...
      const eyebrow=local(content('portfolio'),'eyebrow',locale)||(locale==='pt'?'Evolução de alunos':'Client progress');
    ...
    } else if(results){results.hidden=true;}

    const experience=document.querySelector('#experiencia');
    if(experience){setHeading(experience,locale==='pt'?'Trajetória profissional':'Professional background',locale==='pt'?'Experiência':'Experience');experience.classList.add('fitness-experience-section');}
    const education=document.querySelector('#formacao');
    if(education){setHeading(education,locale==='pt'?'Formação e especializações':'Education and specializations',locale==='pt'?'Credenciais':'Credentials');education.classList.add('fitness-credentials-section');}
    const instagram=document.querySelector('#instagram');
    if(instagram){setHeading(instagram,locale==='pt'?'Treinos, rotina e conteúdo':'Training, routine and content','Instagram');instagram.classList.add('fitness-instagram-section');}
    const contactEl=document.querySelector('#contato');
    if(contactEl){setHeading(contactEl,locale==='pt'?'Fale diretamente comigo':'Talk to me directly',locale==='pt'?'Vamos começar?':'Ready to start?');contactEl.classList.add('fitness-contact-section');}
