(() => {
  ...
  async function currentRole() {
    ...
    const project = await sb.from('projects').select('id,slug').eq('slug', slug).maybeSingle();
  ...
  async function enhanceContent(role) {
    ...
    document.addEventListener('input', event => {
      if (event.target.closest?.('input,textarea,select,[contenteditable="true"]')) markDirty();
    }, true);
