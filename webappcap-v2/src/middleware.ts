...
export async function middleware(request:NextRequest){
  ...
  if(host.kind!=='platform'&&!isPublicAssetPath(path)&&!path.startsWith('/_tenant/')){
  ...
  if(protectedRoute&&!data.user){const target=request.nextUrl.clone();target.pathname='/login';target.searchParams.set('next',path+request.nextUrl.search);return NextResponse.redirect(target)}
  if(path==='/login'&&data.user){const target=request.nextUrl.clone();target.pathname='/entry';target.search='';return NextResponse.redirect(target)}
  return response;
}
