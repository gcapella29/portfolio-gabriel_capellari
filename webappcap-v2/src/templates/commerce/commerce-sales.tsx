...
export function CommerceSalesTemplate({project,data}:TemplateRenderProps){
 ...
 const [customer,setCustomer]=useState({name:'',type:'Entrega',address:'',payment:'',note:''});
 const nextId=useRef(1);
 const defaultSalesWhatsappMessage='Olá! Quero fazer um pedido na {loja}.\n\n👤 Cliente: {cliente}\n📦 Tipo: {tipo}\n{endereco}\n💳 Pagamento: {pagamento}\n\nITENS DO PEDIDO\n{itens}\n\n💰 TOTAL: {total}\n{observacao}',salesWhatsappTemplate=text(data.content,'sales_whatsapp_message',defaultSalesWhatsappMessage);
 ...
 return <div className={`${styles.site} ${polish.polish}`} data-project={project.slug} style={vars}>
  ...
  {cartOpen?<div className={styles.backdrop} role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setCartOpen(false)}}><section className={`${styles.modal} ${styles.cartModal}`} role="dialog" aria-modal="true" aria-labelledby="cart-title"><button className={styles.close} type="button" onClick={()=>setCartOpen(false)} aria-label="Fechar">×</button><h2 id="cart-title">Seu pedido</h2><p>Confira antes de enviar pelo WhatsApp.</p><div className={styles.cartLines}>{cart.map(line=>{const product=products[line.productIndex]||{},lineTotal=(number(text(product,'price'))+line.extras.reduce((sum,item)=>sum+item.price,0))*line.quantity;return <article key={line.id}><div><strong>{line.quantity}x {text(product,'title')}</strong><b>{brl(lineTotal)}</b></div>{line.removed.length?<small>Sem: {line.removed.join(', ')}</small>:null}{line.extras.length?<small>Adicionais: {line.extras.map(item=>item.name).join(', ')}</small>:null}{line.note?<small>Obs.: {line.note}</small>:null}<button type="button" onClick={()=>setCart(current=>current.filter(item=>item.id!==line.id))}>Remover</button></article>})}{!cart.length?<p className={styles.empty}>Seu pedido ainda está vazio.</p>:null}</div>{cart.length?<><div className={styles.checkoutFields}><label><span>Seu nome</span><input value={customer.name} onChange={event=>setCustomer(current=>({...current,name:event.target.value}))}/></label><div className={styles.typeChoice}><button type="button" aria-pressed={customer.type==='Entrega'} onClick={()=>setCustomer(current=>({...current,type:'Entrega'}))}>Entrega</button><button type="button" aria-pressed={customer.type==='Retirada'} onClick={()=>setCustomer(current=>({...current,type:'Retirada'}))}>Retirada</button></div>{customer.type==='Entrega'?<label><span>Endereço</span><input value={customer.address} onChange={event=>setCustomer(current=>({...current,address:event.target.value}))}/></label>:null}<label><span>Forma de pagamento</span><input value={customer.payment} onChange={event=>setCustomer(current=>({...current,payment:event.target.value}))} placeholder="Pix, dinheiro ou cartão"/></label><label><span>Observação geral</span><textarea value={customer.note} onChange={event=>setCustomer(current=>({...current,note:event.target.value}))}/></label></div><div className={styles.cartTotal}><span>Total dos produtos</span><strong>{brl(cartTotal)}</strong></div><a className={styles.confirm} href={`https://wa.me/${whatsapp}?text=${message()}`} target="_blank" rel="noreferrer">Enviar pedido pelo WhatsApp ↗</a></>:null}</section></div>:null}
 </div>;
}
