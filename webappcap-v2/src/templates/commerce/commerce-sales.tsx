...
export function CommerceSalesTemplate({project,data}:TemplateRenderProps){
 ...
 const recordOrder=()=>trackCommerceOrder({projectId:project.id,templateKey:'commerce-sales-1',customerName:customerName.trim(),customerPhone:customerPhone.trim(),total:cartTotal,items:cart.map(line=>{const product=products[line.productIndex]||{},unitPrice=number(text(product,'price'));return{name:text(product,'title',`Produto ${line.productIndex+1}`),quantity:line.quantity,unitPrice,total:commerceLineTotal(product,line.quantity)}})});
