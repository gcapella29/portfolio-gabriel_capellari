...
export function CommerceSalesTemplate({project,data}:TemplateRenderProps){
 ...
 const message=()=>{
  const itemLines=cart.flatMap(line=>{const product=products[line.productIndex]||{},base=[`${line.quantity}x ${text(product,'title')} — ${brl((number(text(product,'price'))+line.extras.reduce((sum,item)=>sum+item.price,0))*line.quantity)}`];if(line.removed.length)base.push(`  ❌ Sem: ${line.removed.join(', ')}`);if(line.extras.length)base.push(`  ➕ Adicionais: ${line.extras.map(item=>`${item.name} (${brl(item.price)})`).join(', ')}`);if(line.note)base.push(`  📝 Obs.: ${line.note}`);return base});
