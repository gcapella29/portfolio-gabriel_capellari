...
function CommerceEditorialTemplate({project,data}:TemplateRenderProps){
 ...
 return <div ref={siteRef} className={`${styles.site} ${theme.palette}`} data-variant={variant} data-scale={scale} data-project={project.slug} style={vars}>
  <header className={`${styles.topbar} ${theme.topbar}`}><a className={styles.wordmark} href="#inicio">{name}</a><nav aria-label="Navegação principal"><a href="#inicio">{copy('nav_home','Início')}</a><a href="#novidades">{copy('nav_news','Novidades')}</a><a href="#destaques">{copy('nav_highlights','Destaques')}</a><a href="#cardapio">{copy('nav_menu',commerce.catalogLabel)}</a><a href="#pedido">{copy('nav_order','Pedido')} <span>{chosen.reduce((sum,item)=>sum+item.quantity,0)}</span></a></nav></header>
  <main>
   ...
   <section className={`${styles.section} ${styles.menu}`} id="cardapio" data-reveal="left"><header className={styles.sectionHead}><span>{copy('menu_kicker',`03 · ${commerce.catalogLabel.toUpperCase()}`)}</span><h2>{copy('menu_title',commerce.catalogLabel)}</h2><p>{copy('menu_intro','Passe sobre um item ou toque para ver todos os detalhes.')}</p></header><div className={styles.menuGrid}><div className={styles.menuRows}>{menu.map((item,index)=>{const title=text(item,'title',`Item ${index+1}`),src=text(item,'image','/commerce/commerce-menu.png');return <button type="button" className={active===index?styles.activeMenu:undefined} onMouseEnter={()=>setActive(index)} onFocus={()=>setActive(index)} onClick={()=>setActive(index)} key={index}><span>{String(index+1).padStart(2,'0')}</span><span className={styles.menuThumb}><Image src={src} alt="" fill sizes="64px"/></span><strong>{title}</strong><small>{text(item,'description')}</small><b>{text(item,'price','R$ 0,00')}</b><i>↗</i></button>})}</div><aside className={styles.menuPreview}><div><Image src={text(menu[active]||{},'image','/commerce/commerce-menu.png')} alt={text(menu[active]||{},'title',`Item · ${commerce.catalogLabel}`)} fill sizes="(max-width: 760px) 92vw, 40vw"/></div><span>{copy('menu_selected_label','SELEÇÃO ATUAL')}</span><h3>{text(menu[active]||{},'title','Escolha um item')}</h3><p>{text(menu[active]||{},'description')}</p><strong>{text(menu[active]||{},'price','')}</strong><button type="button" onClick={()=>change(active,1)}>{copy('menu_add','Adicionar ao pedido +')}</button></aside></div><a className={styles.menuWhatsappCta} href={"https://wa.me/"+whatsapp} target="_blank" rel="noreferrer">{copy("menu_whatsapp_cta","Não encontrou o que queria? Peça pelo WhatsApp!")} ↗</a></section>
