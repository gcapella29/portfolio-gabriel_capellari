...
export function NativePortfolioTemplate({project,data,preview=false}:TemplateRenderProps){
  ...
  return <div ref={siteRef} className={`${styles.site} ${headingFont.variable} ${bodyFont.variable} ${monoFont.variable}`} data-language={language} style={cssVariables}>
    ...
    <main>
      <section className={styles.stats} id="destaques" data-reveal>{stats.map(item=><article key={item.number+item.label.pt}><strong>{item.number}</strong><span>{localized(item.label,language)}</span></article>)}</section>
      ...
      <section className={`${styles.section} ${styles.experience}`} id="experiencia" data-reveal><header><span className={styles.eyebrow}>{language==='pt'?'Histórico de mãos':'Hand history'}</span><h2>{language==='pt'?'Experiência profissional':'Professional experience'}</h2></header><div className={styles.timeline}>{career.map(item=><article key={`${item.organization}-${item.years}`}><div><h3>{localized(item.role,language)}</h3><time>{item.years}</time></div><strong>{item.organization}</strong><p>{localized(item.description,language)}</p></article>)}</div></section>
