...
const storedRows=(record:Record<string,unknown>,key:string)=>{const value=record[key];return Array.isArray(value)?value.filter(item=>item&&typeof item==='object') as Record<string,unknown>[]:null};
...
export function NativePortfolioTemplate({project,data,preview=false}:TemplateRenderProps){
  ...
  const [editorialBeat,setEditorialBeat]=useState(0);
  ...
  const languages=(storedRows(data.content,'portfolio_languages')??defaults.identity.languages.map(item=>({country:item.country,label_pt:item.pt,label_en:item.en}))).map(item=>({country:field(item,'country')||'br',pt:field(item,'label_pt'),en:field(item,'label_en')}));
  const ticker=(storedRows(data.content,'portfolio_ticker')??defaults.ticker.map(title=>({title}))).map(item=>field(item,'title')).filter(Boolean);
  const stats=(storedRows(data.content,'portfolio_stats')??defaults.stats.map(item=>({number:item.number,label_pt:item.label.pt,label_en:item.label.en}))).map(item=>({number:field(item,'number'),label:{pt:field(item,'label_pt'),en:field(item,'label_en')}}));
  const featuredYears=(storedRows(data.content,'portfolio_featured_years')??defaults.featured.years.map(item=>({label_pt:item.pt,label_en:item.en}))).map(item=>({pt:field(item,'label_pt'),en:field(item,'label_en')}));
  const coverage=(storedRows(data.content,'portfolio_coverage')??defaults.coverage.map(([event,years])=>({event,years}))).map(item=>({event:field(item,'event'),years:field(item,'years')}));
  const portfolioLinks=(storedRows(data.content,'portfolio_links')??defaults.links.map(item=>({name:item.name,description_pt:item.description.pt,description_en:item.description.en,url:item.href}))).map(item=>({name:field(item,'name'),description:{pt:field(item,'description_pt'),en:field(item,'description_en')},href:field(item,'url')}));
  const career=(storedRows(data.content,'portfolio_career')??defaults.career.map(item=>({role_pt:item.role.pt,role_en:item.role.en,years:item.years,organization:item.organization,description_pt:item.description.pt,description_en:item.description.en}))).map(item=>({role:{pt:field(item,'role_pt'),en:field(item,'role_en')},years:field(item,'years'),organization:field(item,'organization'),description:{pt:field(item,'description_pt'),en:field(item,'description_en')}}));
  const education=(storedRows(data.content,'portfolio_education')??defaults.education.map(([title,institution])=>({title_pt:title.pt,title_en:title.en,institution}))).map(item=>({title:{pt:field(item,'title_pt'),en:field(item,'title_en')},institution:field(item,'institution')}));
  const skills=(storedRows(data.content,'portfolio_skills')??defaults.skills.map(item=>({label_pt:item.pt,label_en:item.en}))).map(item=>({pt:field(item,'label_pt'),en:field(item,'label_en')}));
  const galleryMeta=(storedRows(data.content,'portfolio_gallery_meta')??defaults.galleryMeta.map(item=>({year:item.year,alt_pt:item.alt.pt,alt_en:item.alt.en}))).map(item=>({year:field(item,'year'),alt:{pt:field(item,'alt_pt'),en:field(item,'alt_en')}}));
  const storedEnglishRole=stringValue(data.content,'hero_text_en',defaults.identity.role.en);
  const role:LocalizedText={pt:stringValue(data.content,'hero_text',defaults.identity.role.pt),en:legacyEnglishRoles.has(storedEnglishRole)?compactPortfolioEnglishRole:storedEnglishRole};
  ...
  const profileBio:LocalizedText={pt:stringValue(data.content,'primary_offer',defaults.profileBio.pt),en:stringValue(data.content,'primary_offer_en',defaults.profileBio.en)};
  ...
  useEffect(()=>{
    ...
    const timer=window.setInterval(()=>{
      if(!document.hidden)setEditorialBeat(current=>current+1);
    },2200);
  ...
  return <div ref={siteRef} className={`${styles.site} ${headingFont.variable} ${bodyFont.variable} ${monoFont.variable}`} data-language={language} data-motion-paused={motionPaused?'true':'false'} style={cssVariables}>
    ...
    <div className={styles.tickerWrap} aria-hidden="true">
      <div className={styles.tickerTrack}>
        {[0,1].map(group=><div className={styles.tickerGroup} key={group}>{ticker.map(item=><span key={`${group}-${item}`}>{item}</span>)}</div>)}
      </div>
    </div>
    <main>
      <section className={styles.stats} id="destaques" data-reveal>{stats.map((item,index)=><article data-editorial-active={editorialBeat%Math.max(stats.length,1)===index} key={item.number+item.label.pt}><strong>{item.number}</strong><span>{localized(item.label,language)}</span></article>)}</section>
