...
const sectionLinks=[['destaques','Destaques'],['sobre','Sobre'],['wsop-featured','WSOP'],['cobertura','Cobertura'],['portfolio','Portfólio'],['experiencia','Experiência'],['formacao','Formação'],['instagram','Instagram'],['contato','Contato']] as const;
...
export function NativePortfolioTemplate({project,data,preview=false}:TemplateRenderProps){
  ...
  const profileImage=mediaValue(data.media,'profile',defaults.media.profile);
  ...
  return <div ref={siteRef} className={`${styles.site} ${headingFont.variable} ${bodyFont.variable} ${monoFont.variable}`} data-language={language} style={cssVariables}>
    ...
    <main>
      ...
      <section className={`${styles.section} ${styles.featured}`} id="wsop-featured" data-reveal><div className={styles.carousel} tabIndex={0} role="region" aria-roledescription="carousel" aria-label={language==='pt'?'Fotos da cobertura da WSOP Las Vegas':'WSOP Las Vegas coverage photos'} onKeyDown={event=>{if(event.key==='ArrowLeft'){event.preventDefault();moveSlide(-1)}if(event.key==='ArrowRight'){event.preventDefault();moveSlide(1)}}} onTouchStart={event=>{touchStartRef.current=event.changedTouches[0].clientX}} onTouchEnd={event=>{if(touchStartRef.current===null)return;const delta=event.changedTouches[0].clientX-touchStartRef.current;touchStartRef.current=null;if(Math.abs(delta)>=45)moveSlide(delta>0?-1:1)}}>{gallery.map((src,index)=>{const meta=defaults.galleryMeta[index]||{year:'2025',alt:`${name} na WSOP Las Vegas`};return <figure className={index===slide?styles.activeSlide:styles.slide} aria-hidden={index!==slide} key={src}><Image src={src} alt={meta.alt} fill sizes="(max-width: 819px) 100vw, 50vw" priority={index===0}/><figcaption><strong>{meta.year}</strong><span>WSOP Las Vegas</span></figcaption></figure>})}<button className={styles.previous} type="button" onClick={()=>moveSlide(-1)} aria-label={language==='pt'?'Foto anterior':'Previous photo'}>←</button><button className={styles.next} type="button" onClick={()=>moveSlide(1)} aria-label={language==='pt'?'Próxima foto':'Next photo'}>→</button><div className={styles.carouselUi}><div className={styles.dots}>{gallery.map((_,index)=><button type="button" className={index===slide?styles.activeDot:styles.dot} aria-label={`${language==='pt'?'Ir para foto':'Go to photo'} ${index+1}`} aria-current={index===slide?'true':undefined} onClick={()=>setSlide(index)} key={index}/>)}</div><div>{String(slide+1).padStart(2,'0')} / {String(gallery.length).padStart(2,'0')}</div></div></div><div className={styles.featuredCopy}><span className={styles.eyebrow}>{localized(defaults.featured.eyebrow,language)}</span><h2>WSOP<br/>Las Vegas</h2><p>{localized(featuredDescription,language)}</p><div className={styles.yearList}>{defaults.featured.years.map(year=><span key={year}>{year}</span>)}</div><a className={styles.workLink} href="#portfolio">{language==='pt'?'Ver portfólio →':'View portfolio →'}</a></div></section>
      ...
      <section className={`${styles.section} ${styles.portfolio}`} id="portfolio" data-reveal><header><span className={styles.eyebrow}>{language==='pt'?'Onde ler':'Where to read'}</span><h2>{language==='pt'?'Portfólio publicado':'Published portfolio'}</h2></header><div className={styles.profile}><div className={styles.profilePhoto}><Image src={profileImage} alt={`${name}, redator na PokerNews Brasil`} fill sizes="220px"/></div><div><h3>{name}</h3><div className={styles.socialRow}><span>{language==='pt'?'Siga no':'Follow on'}</span><a href={`https://www.instagram.com/${instagram}/`} target="_blank" rel="noreferrer" aria-label="Instagram">IG</a><a href={linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn">IN</a></div><p>{localized(profileBio,language)}</p></div></div><div className={styles.pressCards}>{defaults.links.map(item=><a href={item.href} target="_blank" rel="noreferrer" key={item.name}><span><strong>{item.name}</strong><small>{localized(item.description,language)}</small></span><b>→</b></a>)}</div></section>
