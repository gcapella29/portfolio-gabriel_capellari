...
export function PerformanceTrainerTemplate({project,data,preview=false}:TemplateRenderProps){
  ...
  return <div className={`${styles.site} ${tokens.tokens}`} style={cssVars} data-density={density}>
    ...
    <main>
      <section id="inicio" className={`${styles.hero} ${fixes.heroPremium}`}><div className={styles.heroCopy}><span className={styles.kicker}>{specialty}{location?` · ${location}`:''}</span><h1 className={fixes.heroSafe}>{heroTitle}</h1>{heroText&&<p>{heroText}</p>}<div className={styles.heroActions}><a className={styles.primary} href="#contato">Quero começar →</a><a className={styles.secondary} href="#resultados">Ver resultados ↓</a></div><div className={styles.meta}>{location&&<span>{location}</span>}{cref&&<span>{cref}</span>}</div></div><div className={`${styles.heroMedia} ${fixes.heroMediaPremium}`}>{hero?<img src={hero} alt={name}/>:<div className={styles.heroPlaceholder}><span>Imagem principal</span></div>}</div></section>
      {(offer||proof)&&<section className={styles.proofStrip}>{offer&&<strong>{offer}</strong>}{proof&&<p>{proof}</p>}</section>}
      {services.length>0&&<section id="acompanhamento" className={styles.lightSection}><div className={styles.sectionHead}><span>Seu objetivo, seu plano</span><h2>Para quem é o acompanhamento</h2></div><div className={styles.cardGrid}>{services.map((item,i)=><article className={`${styles.serviceCard} ${fixes.premiumCard}`} key={`${item.title}-${i}`}><b>{String(i+1).padStart(2,'0')}</b><h3>{item.title}</h3>{item.text&&<p>{item.text}</p>}</article>)}</div></section>}
      {(gallery.length>0||proof)&&<section id="resultados" className={`${styles.darkSection} ${fixes.premiumDark}`}><div className={styles.sectionHead}><span>Prova social</span><h2>{resultTitle}</h2>{proof&&<p>{proof}</p>}</div>{gallery.length>0&&<div className={`${styles.gallery} ${fixes.premiumGallery}`}>{gallery.map((item,i)=>item?.url?<figure key={`${item.url}-${i}`}><img src={item.url} alt={`Resultado ${i+1}`}/></figure>:null)}</div>}</section>}
      <section className={`${styles.schedule} ${fixes.scheduleSafe}`}><div><span>Agenda</span><h2>{scheduleTitle}</h2><p>{scheduleText}</p></div><a className={styles.scheduleButton} href="#contato">Consultar disponibilidade →</a></section>
