...
export function PerformanceTrainerTemplate({project,data,preview=false}:TemplateRenderProps){
  ...
  return <div className={`${styles.site} ${tokens.tokens}`} style={cssVars} data-density={density}>
    ...
    <main>
      <section id="inicio" className={styles.hero}><div className={styles.heroCopy}><span className={styles.kicker}>{specialty}{location?` · ${location}`:''}</span><h1 className={fixes.heroSafe}>{heroTitle}</h1>{heroText&&<p>{heroText}</p>}<div className={styles.heroActions}><a className={styles.primary} href="#contato">Quero começar →</a><a className={styles.secondary} href="#resultados">Ver resultados ↓</a></div><div className={styles.meta}>{location&&<span>{location}</span>}{cref&&<span>{cref}</span>}</div></div><div className={styles.heroMedia}>{hero?<img src={hero} alt={name}/>:<div className={styles.heroPlaceholder}><span>Imagem principal</span></div>}</div></section>
      ...
      <section id="contato" className={fixes.leadSection}><div className={fixes.leadIntro}><span>Vamos conversar</span><h2>Conte seu objetivo.</h2><p>Preencha os dados abaixo e receba o contato para combinar o melhor formato e horário para o seu acompanhamento.</p>{whatsapp&&<a href={wa(whatsapp)}>Prefere WhatsApp? Fale diretamente →</a>}</div><PersonalTrainerLeadForm projectId={project.id}/></section>
      <section className={styles.finalCta}><span>Próximo passo</span><h2>Pronto para começar?</h2><p>Seu acompanhamento pode começar por aqui. Envie seus dados ou fale diretamente pelo WhatsApp.</p><div>{whatsapp&&<a className={styles.primary} href={wa(whatsapp)}>Falar no WhatsApp →</a>}{instagram&&<a className={styles.secondaryDark} href={instagram.startsWith('http')?instagram:`https://instagram.com/${instagram.replace(/^@/,'')}`}>Instagram ↗</a>}</div></section>
    </main>
