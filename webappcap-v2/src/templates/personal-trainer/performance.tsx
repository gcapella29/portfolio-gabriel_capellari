...
export function PerformanceTrainerTemplate({project,data,preview=false}:TemplateRenderProps){
  ...
  const cssVars={'--pt-accent':accent} as React.CSSProperties;
  ...
  return <div className={styles.site} style={cssVars}>
    ...
    <main>
      <section id="inicio" className={styles.hero}>
        <div className={styles.heroCopy}><span className={styles.kicker}>{specialty}{location?` · ${location}`:''}</span><h1>{heroTitle}</h1>{heroText&&<p>{heroText}</p>}<div className={styles.heroActions}><a className={styles.primary} href={wa(whatsapp)}>Quero começar →</a><a className={styles.secondary} href="#resultados">Ver resultados ↓</a></div><div className={styles.meta}>{location&&<span>{location}</span>}{cref&&<span>{cref}</span>}</div></div>
        <div className={styles.heroMedia}>{hero?<img src={hero} alt={name}/>:<div className={styles.heroPlaceholder}><span>Imagem principal</span></div>}</div>
      </section>
    ...
    <footer className={styles.footer}><strong>{name}</strong><span>{specialty}{cref?` · ${cref}`:''}</span></footer>
    {whatsapp&&<a className={styles.mobileCta} href={wa(whatsapp)}>Consultar horários</a>}
  </div>;
}
