...
export function PersonalTrainerLeadForm({projectId}:{projectId:string}){
  ...
  return <form className={styles.form} onSubmit={submit}>
    ...
    <label className={styles.message}><span>Objetivo</span><textarea name="message" required maxLength={1000} rows={4} placeholder="Conte brevemente o que você busca no acompanhamento."/></label>
    <button type="submit" disabled={status==='sending'}>{status==='sending'?'Enviando...':'Quero começar →'}</button>
    <div className={styles.feedback} aria-live="polite">{status==='success'&&'Recebido! Entraremos em contato pelo WhatsApp.'}{status==='error'&&'Não foi possível enviar agora. Tente novamente ou fale pelo WhatsApp.'}</div>
  </form>;
}
