...
export default async function OwnerProjectsPage({searchParams}:{searchParams:Promise<{deleted?:string}>}){
 ...
 const commerceProjects=projects.filter(project=>(stateByProject.get(project.id)?.segment||project.site_type)==='food-business'),orderEntries=await Promise.all(commerceProjects.map(async project=>[project.id,await commerceOrdersForProject(project.id,{limit:1000})] as const)),ordersByProject=new Map(orderEntries);
 const projectViews:OwnerProjectView[]=projects.map(project=>{const state=stateByProject.get(project.id),projectLeads=leads.filter(lead=>lead.project_id===project.id);return{id:project.id,slug:project.slug,name:project.name,siteType:state?.segment||project.site_type||'portfolio',published:project.is_published===true,lifecycle:state?.lifecycle||(project.is_published?'published':'draft'),onboardingStep:state?.onboarding_step||'completed',templateKey:state?.template_key||null,domainStatus:state?.domain_status||'unconfigured',nativeSubdomain:state?.native_subdomain||null,customDomain:state?.custom_domain||null,updatedAt:state?.updated_at||null,leadsTotal:projectLeads.length,leadsNew:projectLeads.filter(lead=>lead.status==='new').length,ordersTotal:ordersByProject.get(project.id)?.length||0}});
 ...
 const orderActivity=[...ordersByProject.entries()].flatMap(([projectId,orders])=>orders.slice(0,4).map(order=>({key:`order-${projectId}-${order.id}`,date:order.createdAt as string|null,label:'Novo pedido',title:`Pedido · ${new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'}).format(order.total)}`,project:projectById.get(projectId),href:projectById.get(projectId)?`/dashboard/${encodeURIComponent(projectById.get(projectId)!.slug)}/orders`:'#'})));const recentActivity=[...leads.slice(0,8).map(lead=>({key:`lead-${lead.id}`,date:lead.created_at as string|null,label:'Novo lead',title:lead.name||'Contato recebido',project:projectById.get(lead.project_id),href:projectById.get(lead.project_id)?`/dashboard/${encodeURIComponent(projectById.get(lead.project_id)!.slug)}/leads`:'#'})),...orderActivity,...projectViews.filter(project=>project.updatedAt).map(project=>({key:`project-${project.id}`,date:project.updatedAt,label:project.published?'Publicado':'Atualizado',title:project.name,project:projectById.get(project.id),href:project.siteType==='food-business'?`/dashboard/${encodeURIComponent(project.slug)}/editor`:`/dashboard/${encodeURIComponent(project.slug)}/content`}))].sort((a,b)=>new Date(b.date||0).getTime()-new Date(a.date||0).getTime()).slice(0,5);
