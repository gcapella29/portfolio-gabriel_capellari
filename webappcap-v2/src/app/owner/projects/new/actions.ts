...
export async function createClientProject(formData:FormData){
  ...
  const state=await sb.from('project_v2_state').upsert({project_id:project.id,segment,template_key:templateKey,lifecycle:'invited',onboarding_step:'account',domain_status:'unconfigured'},{onConflict:'project_id'});if(state.error)throw state.error;
