...
export default function ContentWorkspace({slug,previewUrl,saved,portfolio,nav,action,children}:{slug:string;previewUrl:string;saved:boolean;portfolio:boolean;nav:ContentNavItem[];action:(formData:FormData)=>Promise<void>;children:React.ReactNode}){
 const formRef=useRef<HTMLFormElement>(null),submitting=useRef(false),[dirty,setDirty]=useState(false),[language,setLanguage]=useState<'pt'|'en'>('pt'),[completion,setCompletion]=useState<Record<string,number>>({});
 ...
 return <div className={styles.workspace} data-language={language}>
  ...
  <div className={styles.editorColumn}>
   ...
   <form ref={formRef} action={action} className={styles.form} onChangeCapture={changed} onClickCapture={event=>{if((event.target as HTMLElement).closest('button[type="button"]'))changed()}} onSubmitCapture={()=>{submitting.current=true}}><input type="hidden" name="slug" value={slug}/>{children}<div className={styles.saveBar}><div><i data-dirty={dirty}/><span>{dirty?'Alterações não salvas':saved?'Rascunho salvo':'Nenhuma alteração pendente'}</span></div><a className="action secondary" href={previewUrl} target="_blank">Abrir Preview ↗</a><SaveButton/></div></form>
  </div>
 </div>;
}
