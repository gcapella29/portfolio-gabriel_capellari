...
export async function saveContentAction(formData:FormData){const slug=slugFrom(formData),access=await resolveProjectAccess(slug);if(!can(access.role,'editContent'))throw new Error('Sem permissão para editar conteúdo.');const current=await readV2Content(access.project.id);await saveV2Section(access.project.id,'identity',{...current.identity,name:text(formData,'name')||access.project.name,tagline:text(formData,'tagline'),description:text(formData,'description'),location:text(formData,'location')});const content={...current.content,...repeatables(formData,access.project.segment),hero_title:text(formData,'hero_title'),hero_text:text(formData,'hero_text'),primary_offer:text(formData,'primary_offer'),proof:text(formData,'proof'),about:text(formData,'about'),extra_notes:text(formData,'extra_notes'),cta_primary:text(formData,'cta_primary'),cta_secondary:text(formData,'cta_secondary'),system_title:text(formData,'system_title'),system_kicker:text(formData,'system_kicker'),system_intro:text(formData,'system_intro'),results_intro:text(formData,'results_intro'),method_title:text(formData,'method_title'),method_intro:text(formData,'method_intro'),final_kicker:text(formData,'final_kicker'),final_title:text(formData,'final_title')};if(access.project.segment==='personal-trainer')Object.assign(content,{trainer_specialty:text(formData,'trainer_specialty'),trainer_cref:text(formData,'trainer_cref'),trainer_services:text(formData,'trainer_services'),trainer_results_title:text(formData,'trainer_results_title'),trainer_schedule_title:text(formData,'trainer_schedule_title'),trainer_schedule_text:text(formData,'trainer_schedule_text'),trainer_method:text(formData,'trainer_method'),trainer_credentials:text(formData,'trainer_credentials')});await saveV2Section(access.project.id,'content',content);await saveV2Section(access.project.id,'contact',{...current.contact,email:text(formData,'email'),phone:text(formData,'phone'),whatsapp:text(formData,'whatsapp'),instagram:text(formData,'instagram'),address:text(formData,'address'),hours:text(formData,'hours')});revalidatePath(path(slug,'content'));revalidatePath(`/preview/${encodeURIComponent(slug)}`);redirect(`${path(slug,'content')}?saved=1`)}
export async function saveAppearanceAction(formData:FormData){const slug=slugFrom(formData),access=await resolveProjectAccess(slug);if(!can(access.role,'editAppearance'))throw new Error('Sem permissão para editar aparência.');const current=await readV2Content(access.project.id);await saveV2Section(access.project.id,'appearance',{...current.appearance,accent:text(formData,'accent')||'#d9ff43',heading_font:text(formData,'heading_font')||'Montserrat',body_font:text(formData,'body_font')||'DM Sans',scale:text(formData,'scale')||'normal',alignment:text(formData,'alignment')||'left',density:text(formData,'density')||'normal'});revalidatePath(path(slug,'appearance'));revalidatePath(`/preview/${encodeURIComponent(slug)}`);redirect(`${path(slug,'appearance')}?saved=1`)}
...
export async function uploadMediaAction(formData:FormData){const slug=slugFrom(formData),access=await resolveProjectAccess(slug);if(!can(access.role,'manageMedia'))throw new Error('Sem permissão para editar mídia.');const current=await readV2Content(access.project.id),media={...current.media};const logo=formData.get('logo'),hero=formData.get('heroImage'),heroVideo=formData.get('heroVideo');if(logo instanceof File&&logo.size)media.logo=await uploadProjectImage(access.project.id,logo,'logo');if(hero instanceof File&&hero.size)media.hero=await uploadProjectImage(access.project.id,hero,'hero');if(heroVideo instanceof File&&heroVideo.size)media.hero_video=await uploadProjectVideo(access.project.id,heroVideo,'hero-video');const files=formData.getAll('gallery').filter(v=>v instanceof File&&v.size) as File[];if(files.length){const old=Array.isArray(media.gallery)?media.gallery:[],uploaded=[];for(const [i,file] of files.slice(0,12).entries())uploaded.push(await uploadProjectImage(access.project.id,file,`gallery-${i+1}`));media.gallery=[...old,...uploaded].slice(-12)}await saveV2Section(access.project.id,'media',media);revalidatePath(path(slug,'media'));revalidatePath(`/preview/${encodeURIComponent(slug)}`);redirect(`${path(slug,'media')}?saved=1`)}
