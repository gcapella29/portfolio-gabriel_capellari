...
export default function SalesContentEditor({slug,data}:{slug:string;data:{identity:Record<string,unknown>;content:Record<string,unknown>;contact:Record<string,unknown>}}){
 const settings=commerceSettings(data.content),menuDefinition=sectionsForSegment('food-business').filter(def=>def.key==='menu_items');
