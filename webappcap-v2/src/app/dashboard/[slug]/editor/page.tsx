...
export default async function TemplateEditorPage({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<{template?:string;savedAppearance?:string;savedContent?:string}>}){
 ...
 return <div className={styles.page}>
  ...
  <section className={styles.realEditor} id="content"><div className={styles.sequenceHead}><span>02 · CONTEÚDO</span><h2>{editor.label}</h2><p>{complete?'Edite toda a estrutura do site em uma única página.':'A Venda rápida reutiliza os mesmos dados do projeto, mas o site exibe somente a estrutura essencial para conversão. Campos não usados continuam preservados caso você volte ao Completo.'}</p></div>{query.savedContent?<div className={styles.success}>Conteúdo salvo no rascunho. Confira no Preview antes de publicar.</div>:null}<div className={styles.embeddedContent}><ContentPage params={Promise.resolve({slug:project.slug})} searchParams={Promise.resolve({embedded:'editor'})}/></div></section>
