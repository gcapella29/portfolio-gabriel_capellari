...
export default async function TemplateEditorPage({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<{template?:string;savedAppearance?:string}>}){
 ...
 const complete=selected==='commerce-main-1';
 return <div className={styles.page}>
  ...
  {complete?<section className={styles.realEditor}><div className={styles.sequenceHead}><span>02 · CONTEÚDO</span><h2>Completo</h2><p>Os formulários reais do Bistrô editorial agora fazem parte desta página. Não é mais necessário entrar em uma tela separada para editar o conteúdo.</p></div><div className={styles.embeddedContent}><ContentPage params={Promise.resolve({slug:project.slug})} searchParams={Promise.resolve({})}/></div></section>:
  <section className={styles.editorSequence}><div className={styles.sequenceHead}><span>02 · EDIÇÃO</span><h2>{editor.label}</h2><p>A estrutura enxuta já está isolada. Os formulários próprios da Venda rápida serão incorporados na próxima rodada.</p></div>{editor.sections.filter(section=>section.key!=='appearance').map((section,index)=><article className={styles.editBlock} key={section.key}><div className={styles.blockNumber}>{String(index+1).padStart(2,'0')}</div><div><h3>{section.label}</h3><p>{section.description}</p></div></article>)}</section>}

  <section className={styles.appearance} id="appearance"><div className={styles.sequenceHead}><span>03 · APARÊNCIA</span><h2>Identidade visual</h2><p>Cores, fontes e proporções ficam no final do mesmo editor.</p></div>{query.savedAppearance?<div className={styles.success}>Aparência salva no rascunho. Confira no Preview antes de publicar.</div>:null}<form action={saveEditorAppearanceAction} className="form-stack"><input type="hidden" name="slug" value={project.slug}/><AppearanceControls initial={appearance}/><div className={styles.saveAppearance}><button className="action primary">Salvar aparência no rascunho</button></div></form></section>
 </div>;
}
