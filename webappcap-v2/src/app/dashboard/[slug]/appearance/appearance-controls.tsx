...
type AppearanceValues={accent:string;scale:string;headingFont:string;bodyFont:string;alignment:string;density:string};
...
export default function AppearanceControls({initial}:{initial:AppearanceValues}){
 ...
 return <div className={styles.customizer}>
  <div className={styles.controls}>
   <section className={styles.controlBlock}><div><span className={styles.label}>Cor de destaque</span><p>Escolha uma sugestão ou defina qualquer cor.</p></div><div className={styles.colorRow}>{swatches.map(color=><button key={color} type="button" className={styles.swatch} aria-label={`Usar cor ${color}`} aria-pressed={accent===color} style={{backgroundColor:color}} onClick={()=>setAccent(color)}/>)}<label className={styles.colorPicker}><input name="accent" type="color" value={accent} onChange={event=>setAccent(event.target.value)}/><span>{accent.toUpperCase()}</span></label></div></section>
