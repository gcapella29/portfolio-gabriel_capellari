...
import { publishDashboardAction } from './actions';

export default async function DashboardLayout({children,params}:{children:React.ReactNode;params:Promise<{slug:string}>}){
  const {slug}=await params;const {project,role}=await resolveProjectAccess(slug);const state=await readProjectState(project.id);const base=`/dashboard/${encodeURIComponent(project.slug)}`;const siteUrl=publicProjectUrl(state,project.slug);
