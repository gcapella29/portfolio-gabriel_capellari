...
export default async function DashboardLayout({children,params}:{children:React.ReactNode;params:Promise<{slug:string}>}){
 ...
 const h=await headers(),host=(h.get('x-forwarded-host')||h.get('host')||'').split(':')[0].toLowerCase(),isolatedHost=host==='localhost'||host==='127.0.0.1'||host.endsWith('.vercel.app'),siteUrl=isolatedHost?`/site/${encodeURIComponent(project.slug)}`:publicProjectUrl(state,project.slug),commerce=project.segment==='food-business';
 const nav=commerce?
