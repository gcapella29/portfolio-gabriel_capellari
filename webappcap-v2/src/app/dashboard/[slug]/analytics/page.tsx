...
export default async function AnalyticsPage({params}:{params:Promise<{slug:string}>}){
 ...
 const events=(eventResult.error?[]:eventResult.data||[]) as EventRow[],leads=commerce?[]:results as LeadRow[],orders=commerce?results as Awaited<ReturnType<typeof commerceOrdersForProject>>:[],views=events.filter(row=>row.event_type==='page_view'),clicks=events.filter(row=>row.event_type!=='page_view'),outcomes=commerce?orders:leads;
 const unique=new Set(views.map(row=>row.session_id).filter(Boolean)).size,conversion=unique?Math.round(outcomes.length/unique*1000)/10:0;
 ...
 const referrers=ranking(views,row=>label(row.referrer_host,'Direto')),pages=ranking(views,row=>pagePath(row.path)),actions=ranking(clicks,row=>eventNames[row.event_type]||row.event_type);
 const daily=Array.from({length:30},(_,index)=>{const date=new Date(now);date.setDate(date.getDate()-index);const key=dayKey(date),dayViews=views.filter(row=>dayKey(row.occurred_at)===key),dayClicks=clicks.filter(row=>dayKey(row.occurred_at)===key),dayResults=commerce?orders.filter(row=>dayKey(row.createdAt)===key):leads.filter(row=>dayKey(row.created_at)===key);return{key,date,views:dayViews.length,visitors:new Set(dayViews.map(row=>row.session_id).filter(Boolean)).size,clicks:dayClicks.length,results:dayResults.length,sources:ranking(dayClicks,row=>label(row.referrer_host,'Direto'),20),actions:ranking(dayClicks,row=>eventNames[row.event_type]||row.event_type,20)}}).filter(day=>day.views||day.clicks||day.results);
 return <div className={styles.analyticsPage}>
  ...
  {eventResult.error?<div className={styles.notice}>O rastreamento de visitas ainda não está disponível neste ambiente.</div>:null}
  <section className={styles.analyticsStats}><article><span>Visualizações</span><strong>{views.length}</strong></article><article><span>Visitantes</span><strong>{unique}</strong></article><article><span>{commerce?'Pedidos':'Leads'}</span><strong>{outcomes.length}</strong></article><article><span>{commerce?'Conversão em pedido':'Conversão em lead'}</span><strong>{conversion}%</strong></article></section>
  ...
  <section className={`${styles.analyticsPanel} ${styles.dailyPanel}`}><div className={styles.panelTitle}><span>HISTÓRICO DIÁRIO</span><strong>Visitas, cliques e {commerce?'pedidos':'leads'} por dia</strong></div><div className={styles.dailyList}>{daily.length?daily.map(day=><details key={day.key}><summary><time dateTime={day.key}>{dayLabel.format(day.date).replaceAll('.','')}</time><span><b>{day.visitors}</b> visitantes</span><span><b>{day.views}</b> visualizações</span><span><b>{day.clicks}</b> cliques</span><span><b>{day.results}</b> {commerce?'pedidos':'leads'}</span><i aria-hidden="true">+</i></summary><div className={styles.dailyDetail}><div><small>ORIGEM DOS CLIQUES</small>{day.sources.length?day.sources.map(([source,count])=><p key={source}><span>{source}</span><strong>{count}</strong></p>):<p><span>Nenhum clique neste dia</span><strong>0</strong></p>}</div><div><small>AÇÕES REALIZADAS</small>{day.actions.length?day.actions.map(([action,count])=><p key={action}><span>{action}</span><strong>{count}</strong></p>):<p><span>Nenhuma interação</span><strong>0</strong></p>}</div></div></details>):<p className={styles.dailyEmpty}>Os detalhes aparecerão assim que o site receber visitas.</p>}</div></section>
