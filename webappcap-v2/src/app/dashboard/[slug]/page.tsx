...
export default async function DashboardPage({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<{published?:string;publishError?:string}>}){
 ...
 const segment=segments[project.segment],pending=pub.hasPendingChanges;
 const statusLabel=pending?'Alterações pendentes':project.isPublished?'Publicado':'Rascunho',dotClass=pending?styles.statusDotPending:project.isPublished?styles.statusDotLive:styles.statusDotDraft;
 return <>
  <section className={styles.hero}><div><span className={styles.eyebrow}>{segment.name}</span><h1>Visão geral</h1><p>{pending?'Há alterações para revisar antes da próxima publicação.':project.isPublished?'Site online e sincronizado.':'Finalize o projeto para fazer a primeira publicação.'}</p></div><div className={styles.status}><i className={dotClass}/>{statusLabel}</div></section>
  ...
  <section className={styles.summary} aria-label="Resumo do projeto"><article><span>Domínio</span><strong>{state?.custom_domain&&state.domain_status==='active'?state.custom_domain:state?.native_subdomain?`${state.native_subdomain}.webappcap.com.br`:'Não configurado'}</strong></article>{leadAccess?<article><span>Leads novos</span><strong>{newResult.count||0}</strong></article>:null}<article><span>Última publicação</span><strong>{pub.publishedAt?new Intl.DateTimeFormat('pt-BR',{dateStyle:'short',timeStyle:'short'}).format(new Date(pub.publishedAt)):'Ainda não publicado'}</strong></article></section>
 </>;
}
