...
export async function publishV2Project(projectId:string){
  ...
  const b=await sb.from('projects').update({is_published:true}).eq('id',projectId).select('slug').maybeSingle();if(b.error)throw b.error;
  invalidatePublicSiteCache({
    slug:b.data?.slug||null,
    nativeSubdomain:state.data.native_subdomain||b.data?.slug||null,
    customDomain:state.data.custom_domain||null
  });
}
