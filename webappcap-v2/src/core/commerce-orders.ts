...
export type CommerceOrder={id:string;templateKey:string;items:CommerceOrderItem[];total:number;createdAt:string;customerName?:string;customerPhone?:string};
...
function parseFallback(row:{id:number;name:string|null;phone:string|null;message:string|null;created_at:string}):CommerceOrder|null{
 ...
 try{
  ...
  return{id:`legacy-${row.id}`,templateKey:String(raw.t||'commerce-main-1'),items,total:Number(raw.x)||0,createdAt:row.created_at,customerName:String(row.name||'').trim()||undefined,customerPhone:String(row.phone||'').trim()||undefined};
 }catch{return null}
}
